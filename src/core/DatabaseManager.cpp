    ...
    {   // === 锁的作用域开始 ===
        ...
        if (!updated) {
            ...
            if (query.exec()) {
                ...
                syncFts(lastId.toInt(), title, plainContent);
...
bool DatabaseManager::updateNote(int id, const QString& title, const QString& content, const QStringList& tags,
...

    {
        ...
        success = query.exec();
        if (success) syncFts(id, title, plainContent);
    }
...
bool DatabaseManager::updateNoteState(int id, const QString& column, const QVariant& value) {
    bool success = false;
    QString currentTime = QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss");
    QString syncTitle, syncContent;
    bool needFts = (column == "content" || column == "title");

    {
        ...
        if (success && needFts) {
            ...
            if (fetch.exec() && fetch.next()) {
                syncTitle = fetch.value(0).toString();
                syncContent = fetch.value(1).toString();
            }
        }
    } 

    if (success && needFts) {
        QString plain = stripHtml(syncContent);
        QMutexLocker locker(&m_mutex);
        syncFts(id, syncTitle, plain);
    }
