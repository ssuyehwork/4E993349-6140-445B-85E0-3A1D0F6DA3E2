#include "AES.h"
#include <cstring>
#include <algorithm>

// S-Box, Inverse S-Box, Rcon... (Standard AES lookup tables)
static const uint8_t sbox[256] = {
  
};

static const uint8_t rsbox[256] = {
  
};

static const uint8_t Rcon[11] = {
  0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

AES::AES(KeyLength keyLength) {
    m_nk = keyLength / 4;
    m_nb = 4;
    if (keyLength == AES_128) m_nr = 10;
    else if (keyLength == AES_192) m_nr = 12;
    else m_nr = 14;
}

AES::~AES() {}

void AES::keyExpansion(const uint8_t* key, uint8_t* w) {
    uint8_t temp[4];
    for (int i = 0; i < m_nk; ++i) {
        w[4 * i] = key[4 * i];
        w[4 * i + 1] = key[4 * i + 1];
        w[4 * i + 2] = key[4 * i + 2];
        w[4 * i + 3] = key[4 * i + 3];
    }
    for (int i = m_nk; i < m_nb * (m_nr + 1); ++i) {
        temp[0] = w[4 * (i - 1)];
        temp[1] = w[4 * (i - 1) + 1];
        temp[2] = w[4 * (i - 1) + 2];
        temp[3] = w[4 * (i - 1) + 3];
        if (i % m_nk == 0) {
            uint8_t t = temp[0];
            temp[0] = sbox[temp[1]] ^ Rcon[i / m_nk];
            temp[1] = sbox[temp[2]];
            temp[2] = sbox[temp[3]];
            temp[3] = sbox[t];
        } else if (m_nk > 6 && i % m_nk == 4) {
            temp[0] = sbox[temp[0]];
            temp[1] = sbox[temp[1]];
            temp[2] = sbox[temp[2]];
            temp[3] = sbox[temp[3]];
        }
        w[4 * i] = w[4 * (i - m_nk)] ^ temp[0];
        w[4 * i + 1] = w[4 * (i - m_nk) + 1] ^ temp[1];
        w[4 * i + 2] = w[4 * (i - m_nk) + 2] ^ temp[2];
        w[4 * i + 3] = w[4 * (i - m_nk) + 3] ^ temp[3];
    }
}

static uint8_t gmul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    for (int i = 0; i < 8; ++i) {
        if (b & 1) p ^= a;
        bool hi_bit_set = (a & 0x80);
        a <<= 1;
        if (hi_bit_set) a ^= 0x1b;
        b >>= 1;
    }
    return p;
}

void AES::cipher(const uint8_t in[16], uint8_t out[16], const uint8_t* w) {
    uint8_t state[4][4];
    for (int i = 0; i < 4; ++i)
        for (int j = 0; j < 4; ++j)
            state[j][i] = in[i * 4 + j] ^ w[i * 4 + j];

    for (int round = 1; round < m_nr; ++round) {
        for (int i = 0; i < 4; ++i)
            for (int j = 0; j < 4; ++j)
                state[i][j] = sbox[state[i][j]];

        uint8_t temp = state[1][0];
        state[1][0] = state[1][1]; state[1][1] = state[1][2]; state[1][2] = state[1][3]; state[1][3] = temp;
        temp = state[2][0]; uint8_t temp2 = state[2][1];
        state[2][0] = state[2][2]; state[2][1] = state[2][3]; state[2][2] = temp; state[2][3] = temp2;
        temp = state[3][0]; temp2 = state[3][1]; uint8_t temp3 = state[3][2];
        state[3][0] = state[3][3]; state[3][1] = temp; state[3][2] = temp2; state[3][3] = temp3;

        for (int j = 0; j < 4; ++j) {
            uint8_t a = state[0][j], b = state[1][j], c = state[2][j], d = state[3][j];
            state[0][j] = gmul(a, 2) ^ gmul(b, 3) ^ c ^ d;
            state[1][j] = a ^ gmul(b, 2) ^ gmul(c, 3) ^ d;
            state[2][j] = a ^ b ^ gmul(c, 2) ^ gmul(d, 3);
            state[3][j] = gmul(a, 3) ^ b ^ c ^ gmul(d, 2);
        }

        for (int i = 0; i < 4; ++i)
            for (int j = 0; j < 4; ++j)
                state[j][i] ^= w[round * 16 + i * 4 + j];
    }

    for (int i = 0; i < 4; ++i)
        for (int j = 0; j < 4; ++j)
            state[i][j] = sbox[state[i][j]];

    uint8_t temp = state[1][0];
    state[1][0] = state[1][1]; state[1][1] = state[1][2]; state[1][2] = state[1][3]; state[1][3] = temp;
    temp = state[2][0]; uint8_t temp2 = state[2][1];
    state[2][0] = state[2][2]; state[2][1] = state[2][3]; state[2][2] = temp; state[2][3] = temp2;
    temp = state[3][0]; temp2 = state[3][1]; uint8_t temp3 = state[3][2];
    state[3][0] = state[3][3]; state[3][1] = temp; state[3][2] = temp2; state[3][3] = temp3;

    for (int i = 0; i < 4; ++i)
        for (int j = 0; j < 4; ++j)
            out[i * 4 + j] = state[j][i] ^ w[m_nr * 16 + i * 4 + j];
}

void AES::invCipher(const uint8_t in[16], uint8_t out[16], const uint8_t* w) {
    uint8_t state[4][4];
    for (int i = 0; i < 4; ++i)
        for (int j = 0; j < 4; ++j)
            state[j][i] = in[i * 4 + j] ^ w[m_nr * 16 + i * 4 + j];

    for (int round = m_nr - 1; round > 0; --round) {
        uint8_t temp = state[1][3];
        state[1][3] = state[1][2]; state[1][2] = state[1][1]; state[1][1] = state[1][0]; state[1][0] = temp;
        temp = state[2][2]; uint8_t temp2 = state[2][3];
        state[2][2] = state[2][0]; state[2][3] = state[2][1]; state[2][0] = temp; state[2][1] = temp2;
        temp = state[3][0]; temp2 = state[3][1]; uint8_t temp3 = state[3][2];
        state[3][0] = state[3][1]; state[3][1] = state[3][2]; state[3][2] = state[3][3]; state[3][3] = temp;

        for (int i = 0; i < 4; ++i)
            for (int j = 0; j < 4; ++j)
                state[i][j] = rsbox[state[i][j]];

        for (int i = 0; i < 4; ++i)
            for (int j = 0; j < 4; ++j)
                state[j][i] ^= w[round * 16 + i * 4 + j];

        for (int j = 0; j < 4; ++j) {
            uint8_t a = state[0][j], b = state[1][j], c = state[2][j], d = state[3][j];
            state[0][j] = gmul(a, 14) ^ gmul(b, 11) ^ gmul(c, 13) ^ gmul(d, 9);
            state[1][j] = gmul(a, 9) ^ gmul(b, 14) ^ gmul(c, 11) ^ gmul(d, 13);
            state[2][j] = gmul(a, 13) ^ gmul(b, 9) ^ gmul(c, 14) ^ gmul(d, 11);
            state[3][j] = gmul(a, 11) ^ gmul(b, 13) ^ gmul(c, 9) ^ gmul(d, 14);
        }
    }

    uint8_t temp = state[1][3];
    state[1][3] = state[1][2]; state[1][2] = state[1][1]; state[1][1] = state[1][0]; state[1][0] = temp;
    temp = state[2][2]; uint8_t temp2 = state[2][3];
    state[2][2] = state[2][0]; state[2][3] = state[2][1]; state[2][0] = temp; state[2][1] = temp2;
    temp = state[3][0]; temp2 = state[3][1]; uint8_t temp3 = state[3][2];
    state[3][0] = state[3][1]; state[3][1] = state[3][2]; state[3][2] = state[3][3]; state[3][3] = temp;

    for (int i = 0; i < 4; ++i)
        for (int j = 0; j < 4; ++j)
            state[i][j] = rsbox[state[i][j]];

    for (int i = 0; i < 4; ++i)
        for (int j = 0; j < 4; ++j)
            out[i * 4 + j] = state[j][i] ^ w[i * 4 + j];
}

std::vector<uint8_t> AES::encryptCBC(const std::vector<uint8_t>& input, const std::vector<uint8_t>& key, const std::vector<uint8_t>& iv) {
    std::vector<uint8_t> w(m_nb * (m_nr + 1) * 4);
    keyExpansion(key.data(), w.data());

    // PKCS#7 Padding
    size_t paddingLen = 16 - (input.size() % 16);
    std::vector<uint8_t> paddedInput = input;
    for (size_t i = 0; i < paddingLen; ++i) paddedInput.push_back((uint8_t)paddingLen);

    std::vector<uint8_t> output(paddedInput.size());
    uint8_t prevBlock[16];
    memcpy(prevBlock, iv.data(), 16);

    for (size_t i = 0; i < paddedInput.size(); i += 16) {
        uint8_t block[16];
        for (int j = 0; j < 16; ++j) block[j] = paddedInput[i + j] ^ prevBlock[j];
        cipher(block, &output[i], w.data());
        memcpy(prevBlock, &output[i], 16);
    }
    return output;
}

std::vector<uint8_t> AES::decryptCBC(const std::vector<uint8_t>& input, const std::vector<uint8_t>& key, const std::vector<uint8_t>& iv) {
    if (input.empty() || input.size() % 16 != 0) return {};
    std::vector<uint8_t> w(m_nb * (m_nr + 1) * 4);
    keyExpansion(key.data(), w.data());

    std::vector<uint8_t> output(input.size());
    uint8_t prevBlock[16];
    memcpy(prevBlock, iv.data(), 16);

    for (size_t i = 0; i < input.size(); i += 16) {
        uint8_t block[16];
        invCipher(&input[i], block, w.data());
        for (int j = 0; j < 16; ++j) output[i + j] = block[j] ^ prevBlock[j];
        memcpy(prevBlock, &input[i], 16);
    }

    // PKCS#7 Unpadding
    uint8_t paddingLen = output.back();
    if (paddingLen > 0 && paddingLen <= 16) {
        bool valid = true;
        for (size_t i = 0; i < paddingLen; ++i) {
            if (output[output.size() - 1 - i] != paddingLen) { valid = false; break; }
        }
        if (valid) output.resize(output.size() - paddingLen);
    }
    return output;
}
